...
public class BookMessageSenderBean implements BookMessageSenderLocal {
   ...
   public void order(boolean sendAsText,String isbn,String name){
       OrderVO orderVO = new OrderVO(isbn, name);
        try {
    ...
    private Message createJMSMessageForjmsServiceFacade(boolean sendAsText,Session session, OrderVO orderVO) throws JMSException {
        if(sendAsText)
    ...
    private void sendJMSMessageToServiceFacade(boolean sendAsText,OrderVO messageData) throws JMSException {
